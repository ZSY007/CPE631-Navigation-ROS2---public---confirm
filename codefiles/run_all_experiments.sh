...
EXPERIMENT_TIMEOUT="${EXPERIMENT_TIMEOUT:-720}"
...
reset_pedestrians() {
    ...
    log "重置行人到初始位置..."
    if timeout 8 ros2 service call /reset_pedestrians std_srvs/srv/Trigger "{}" >/tmp/reset_pedestrians.log 2>&1; then
        ok "行人已重置"
        sleep 2
        return 0
    fi

    warn "行人重置服务调用失败 (非致命)，查看 /tmp/reset_pedestrians.log"
    return 1
}
...
start_simulation() {
    ...
    if ! kill -0 "$LAUNCH_PID" 2>/dev/null; then
        err "launch 已退出"
        warn "查看日志: cat /tmp/launch_${mode}.log"
        return 1
    fi
    ok "仿真已启动"
...
run_single_experiment() {
    ...
    local collector_log="/tmp/collector_${mode}_${experiment_id}_${attempt}.log"
    local sender_log="/tmp/sender_${mode}_${experiment_id}_${attempt}.log"
    local result_file="/tmp/result_${mode}_${experiment_id}_${attempt}.txt"
...
echo -e "${BOLD}║  日志目录: ${NC}/tmp/launch_*.log, /tmp/collector_*.log, /tmp/sender_*.log"
echo -e "${BOLD}╚═══════════════════════════════════════════════════════╝${NC}"
